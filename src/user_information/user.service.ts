...
export class UserService {
   ...
   * @returns Promise<Video[]> - A promise resolving to an array of video objects
   */
  async getPrivateVideos(id: string): Promise<TypeOfVideos[] | string[]> {
    const user = await this.userModel.findById(id).populate('videos');
    return user.videos;
  }
  ...
  async sendEmail(email: string): Promise<string> {
    try {
      ...
      const message = {
        ...
        to: email,
        subject: `Virtual Space Web App: Forgot Password`,
        text: `Welcome to Virtual Space site.`,
        html: `<p>
