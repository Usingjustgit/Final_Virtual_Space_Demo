...
export class VideoServices {
   ...
   * @param email This user email to find the user which user video
   * @returns Promise<Videos[]> - A promise resolving to an array of videos
   */
  async getVideo(email: string) {
    try {
      // const user = (await this.userService.findUserByEmail(email)).populate('videos');
      // return user.videos;
      return 'hello';
    } catch (error) {
  ...
  async uploadVideoInfo(email: string, VideoInfo: TypeVideos) {
    try {
      const { video_title, video_description, video_url, isPublic } = VideoInfo;
      const user = await this.userService.findUserByEmail(email);
      const video_id = await new this.videoModel({
        video_title,
        video_description,
        video_url,
        isPublic,
  ...
  async deleteVideoInfo(user_email: string, video_id: string) {
    try {
      ...
      currentUser.videos.filter((video) => {
        if (video != video_id) {
          return video;
        }
      });
